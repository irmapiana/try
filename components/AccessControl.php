...
class AccessControl extends \yii\base\ActionFilter
{
    ...
    public function beforeAction($action)
    {
        ...
        $actionId = $action->getUniqueId();
        if (!$user->getIsGuest()) {
            // // $dataWebgrp = Muser::find()->where(['USERID' => Yii::$app->user->identity->username])->one();
            // var_dump(Yii::$app->user->identity->webgroup_user);
            // die();
            $role = Yii::$app->user->identity->webgroup_user;
            //$role = Yii::$app->user->identity->role;
            if (Polisitoba::boi(Yii::$app->controller->id.'/' . Yii::$app->controller->action->id,$role)) {
